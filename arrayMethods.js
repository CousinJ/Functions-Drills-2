...
// CODE HERE
const evenNumbers  = mixedNumbers.filter(function(el) {
  if(el % 2 === 0) {
    return el
  }
})
...
// CODE HERE
const postTaxPrices = prices.map((el) => el * 1.07);
...
// CODE HERE
const totalPopulation = populations.reduce((ac, curr) => ac + curr);
...
const myStrongest  = monstersInYourPocket.filter(monstersInYourPocket => monstersInYourPocket.CP > 200);
...
// CODE HERE
const totals = orders.map(orders => orders.price * (orders.tax + 1))
console.log(totals)
...
*/const sum = purchases.filter(purchases343 => purchases343.owner === "Bob").map(purchases => purchases.price).reduce((x, y) => x + y)
console.log(sum)
